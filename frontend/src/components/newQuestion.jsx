...
import API_URL, { makeQuestion, makeAnswer } from '../constants';
...
const NewQuestion = ({ id, open, handleClose }) => {
  ...
  const handleSave = () => {
    ...
    // make a new question
    const answer = [
      makeAnswer(uuid(), answer1, checked1),
      makeAnswer(uuid(), answer2, checked2),
      makeAnswer(uuid(), answer3, checked3),
      makeAnswer(uuid(), answer4, checked4),
      makeAnswer(uuid(), answer5, checked5),
      makeAnswer(uuid(), answer6, checked6),
    ];
  ...
  const drawer = (
    <div>
      ...
      <Grid container spacing={2} style={{ flexGrow: 1, marginLeft: 1 }}>
        <Grid item xs={6}>
          <Button data-test-target='ExitNewQuestionEditPage' variant="contained" style={{ backgroundColor: 'red', color: 'white' }} onClick={handleClose}>
            Exit
          </Button>
        </Grid>
        <Grid item xs={6}>
